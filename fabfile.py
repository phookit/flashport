...
env.db_sqlite = conf.get("DB_SQLITE", False)
...
env.proj_path = "/home/%s/webapps/%s/%s" % (env.user, env.proj_name, env.proj_name)
...
def upload_template_and_reload(name):
    ...
    with open(local_path, "r") as f:
        ...
        if "%(db_pass)s" in local_data and not env.db_sqlite:
            env.db_pass = db_pass()
        local_data %= env
