...
env.venv_home = conf.get("VIRTUALENV_HOME", "/home/%s/webapps/%s" % (env.user, env.proj_name))
env.venv_path = "%s" % (env.venv_home)
env.proj_dirname = "%s" % env.proj_name
env.proj_path = "%s/%s" % (env.venv_path, env.proj_dirname)
env.manage = "%s/bin/python %s/%s/manage.py" % (env.venv_pathi, env.proj_name)
